...
Отчет создаваемый через VIEW берет для расчетов текущие цены товара, которые могут меняться в будущем. Так, если цена условной банки 
кукурузы вырастет, то отчет за весь период покажет доход, которого никогда не было.

В отличие от отчета наше решение на основе таблицы `good_sum_mart` будет корректно отрабатывать изменение цен и новые продажи, но оно не 
лишено недостатков. Так, например, если поменять цену товара, а потом изменить или удалить записи, созданные до этого изменения, то вся 
математика отчета будет нарушена.

Для более надежного решения требуется завести таблицу с историей цен, которая содержит периоды для каждой стоимости товара. Для ее 
наполнения потребуется триггер для таблицы `goods`. Далее модифицировать триггерную функцию таблицы `sales`, чтобы она учитывала 
цену товара, которая есть или была на момент совершения продажи.
