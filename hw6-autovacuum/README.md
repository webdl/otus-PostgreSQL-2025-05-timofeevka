...
Посмотрит на текущее количество "мертвых" записей в таблице с помощью запроса ниже. Сейчас он не покажет записей, но пригодится позднее, 
...
(1 row)
```
Подождать некоторое время, пока не пройдет autovacuum и повторить запрос:
```
 relname | n_live_tup | n_dead_tup | ratio% |        last_autovacuum        
---------+------------+------------+--------+-------------------------------
...
Схожим образом работает команда DELETE - записи не удаляются, а помечаются специальным флагом. Таким образом команда Update - это 
...
VACUUM ANALYZE bigdata; -- собрать статистику по таблице и почистить мертвые записи
...
### Способ №1: Принудительная дефрагментация (vacuum full)
