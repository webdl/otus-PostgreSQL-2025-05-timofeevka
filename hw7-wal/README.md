...
Перед началом работ следует очистить каталог с текущими wal-файлами, если используется не свежеустановленный сервер. Это нужно чтобы в 
ходе выполнения ДЗ точно понимать, сколько wal-файлов будет задействовано.
...
### Вариант №1: Сокращение количества wal-файлов с помощью функции
...
После выполнения функции в каталоге `/mnt/pg_data/main/pg_wal` оказалось всего 12 файлов общим весом 192Mb.
...
### Вариант №2: Сокращение количества wal-файлов с pgbench
...
## Настройка частоты выполнения контрольных точек (checkpoint)
...
## Замеры
Наша задача измерить, какой объем wal-файлов будет сгенерирован при работе pgbench в течение 10 минут, а также вычислить среднее количество 
wal-файлов, используемых между контрольными точками.
